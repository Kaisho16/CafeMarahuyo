...
namespace OrderService.Controllers
{
    ...
    public class OrderController : ControllerBase
    {
        ...
        public OrderController(PosDbContext context, CafeDbContext cafeContext)
        {
            _context = context;
        ...
        public async Task<IActionResult> Checkout([FromBody] CreateOrderRequest req)
        {
            ...
            try
            {
                ...
                foreach (var itemReq in req.Items)
                {
                    ...
                    {
                        int quantityToDeduct = ingredient.QuantityRequired * itemReq.Quantity;
                        
                        var invItem = await _cafeContext.InventoryItems
                            .Include(i => i.Batches)
                        ...
                        if (invItem != null && invItem.Quantity >= quantityToDeduct)
                        {
                            ...
                            foreach (var batch in orderedBatches)
                            {
                                ...
                                if (batch.Quantity > 0)
                                {
                                    if (primaryBatch == null) primaryBatch = batch;
                            ...
                            int userId = 1; // Default or parsed from token
                            if (int.TryParse(User.FindFirstValue("id"), out int parsedUserId))
                                userId = parsedUserId;

                            _cafeContext.Transactions.Add(new Transaction
                            {
                                ItemId = invItem.Id,
                                Type = "stock_out",
                            ...
                            _cafeContext.InventoryBatches.RemoveRange(emptyBatches);
                        }
                    }
                }
        ...
        {
            var logs = await _context.ProductAuditLogs.OrderByDescending(l => l.Timestamp).Take(100).ToListAsync();
        ...
        {
            var cashierName = User.FindFirstValue("displayName") ?? "Unknown";
            _context.ProductAuditLogs.Add(new ProductAuditLog
            {
                Action = action,
